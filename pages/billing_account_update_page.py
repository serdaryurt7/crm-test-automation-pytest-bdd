...
class BillingAccountUpdatePage(BillingAccountCreatePage):
    # "Fatura Hesabını Düzenle" formu, "Fatura Hesabı Oluştur" formuyla
    # BİREBİR AYNI bileşeni kullanıyor (canlı doğrulandı: aynı account-name/
    # -description/address-card* locator'ları, tek fark Kaydet butonunun
    # metni ve alanların önceden dolu gelmesi) - gerçek bir "is-a" ilişkisi
    # olduğundan BillingAccountCreatePage'den türetildi (AddressUpdatePage/
    # AddressAddPage zincirindeki AYNI mantık).
    ...
    def is_edit_form_open(self):
        # Temel sınıftaki is_create_form_open() ile AYNI DOM kontrolünü
        # (account-name input varlığı) yapıyor - Create/Edit ayrımı sadece
        # HANGİ butonla açıldığına bağlı, form bileşeni aynı olduğundan
        # mantığı TEKRARLAMADAN sadece daha okunur bir isimle sarmalıyor.
    ...
    def switch_to_alternate_address(self):
        # Hesap Adı'na DOKUNMUYOR - sadece hizmet adresi değiştiriliyor
        # (TC-011-04, isim değişikliğinden bağımsız olarak test ediyor).
        self._switch_to_first_unselected_address()
    ...
    def _switch_to_first_unselected_address(self):
        ...
        # click_edit_on_row() TEKRAR çağrıldığında (verify_new_address_
        # persisted içinde, kalıcılığı doğrulamak için) _pre_edit_* alanları
        # EZİLİYOR - bu yüzden "beklenen" değer AYRI bir isimde (_expected_
        # service_address_title) saklanıyor, reopen sonrası karşılaştırma
        # kendi kendiyle değil GERÇEK beklenenle yapılabilsin diye.
    ...
    def verify_new_address_persisted(self):
        # Hizmet adresi satırda GÖRÜNTÜLENMİYOR (canlı doğrulandı, UC-010
        # keşfinde) - bu yüzden kalıcılığı, formu TEKRAR açıp seçili
        # adresin GERÇEKTEN yeni adres olduğunu kontrol ederek doğruluyoruz
        # (address_add/address_delete'teki reload-tabanlı kalıcılık
        # deseniyle AYNI mantık: gözlemlenebilir sonucu doğrula).
